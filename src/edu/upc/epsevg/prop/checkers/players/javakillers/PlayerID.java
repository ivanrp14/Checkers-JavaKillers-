...
private double MiniMax_r(GameStatus gs, int d, double alfa, double beta, long hash) {
    ...
    if (timeOut) {
        if (n != null) {
            if (d == 0 && n.type == Node.nType.EXACT) return n.eval;
            else if (gs.getCurrentPlayer() == maximizerPlayer && n.type == Node.nType.ALFA && n.depth >= d) return n.eval;
            else if (gs.getCurrentPlayer() == minimizerPlayer && n.type == Node.nType.BETA && n.depth >= d) return n.eval;
        } else {
            if (gs.getCurrentPlayer() == maximizerPlayer) return Double.NEGATIVE_INFINITY;
            else {
